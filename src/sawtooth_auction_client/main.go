...
const payload = "item1,addItem,giocattolo,bellissimo,posttime,exp,alfredo,timestamp,400,bidid1"
...
func GetClient(args Command, readFile bool) (AuctionClient, error) {
	url := args.UrlPassed()
	if url == "" {
		url = DEFAULT_URL
	}
	...
	if readFile {
		...
		if err != nil {
			return AuctionClient{}, err
		}
	}
	return NewAuctionClient(url, keyfile)
}
...
func GetKeyfile(keyfile string) (string, error) {
	if keyfile == "" {
		...
		return path.Join(
			username.HomeDir, ".sawtooth", "keys", "root"+".priv"), nil
	} else {
		return keyfile, nil
	}
}
